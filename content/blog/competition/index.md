...
# Competition Details
The purpose of this competition is to build a defensive malware detection system (static analysis only) specifically for Windows PE files, and in the second stage of the competition to develop adversarial malware samples that are able to evade other teams' malware detection systems.
...
Evasive sample execution in sandbox must be equivalent to the original sample.
```
...
Send to Mike if Zoe's threshold is in the range [60-64%]
...
Why did we go with two RFCs? In a lot of the papers that we read, RFCs are widely used for malware detection, and much more popular than Neural networks, since they can introduce non-linear relationships. In addition, we tried out a lot of other models, but they did not really work
...
- More datasets → hardware limitations, we were building all of our models on our local computers rather than in the cloud leading us to very low-performance machines
...
- Imbalanced datasets → scared of the Zoo: We found a great dataset called [theZoo](https://github.com/ytisf/theZoo), but we did not want to clone this repo on our computers since it was live malware, and at the time we did not have VMs setup.
...
So we opted to just directly convert the malware sample to an image, and save it as a resource in a dropper. Then when the file is executed, the image is read, uncompressed, and then dropped as an exe (or dll). Here is an example of what malware looks like as an image:
![](0038_L.png)

Initially, when developing this approach we used [OpenCV](https://opencv.org/) to convert the image back to a PE file, but the adversarial was not functional since it relied on the OpenCV dll (which is not available on every single Windows computer). So then we made use of an opensource library called [lodepng](https://github.com/lvandeve/lodepng) and this [gist](https://gist.github.com/Neboer/f47a7fa2006650e9350a384225f94a4f) which were completely self-contained to convert the image back to a PE file.

The reason we did not use this attack is when testing against some of the other teams' models this approach ended up performing much worse than simply adding the original malware file directly as a resource!

Another idea we had was to embed the malware as the application icon for the dropper, but we simply could not get around to it. Here you cannot use a png file type, you would need to convert the malware PE file to an `.ico` file.

## Breaking In! (WhiteBoxing)
Most teams (including ourselves) only provided the final result of our models, either a 1 indicating malware or 0 indicating goodware classification for the submitted file. However, when developing attacks seeing more details, such as the soft labels will allow us to see how *well* our attacks are performing (is there a huge decrease in confidence, or is it very small)
...
# Fun Facts
- You *cannot* `bash` into our docker image (that is intentional on our part), since (at the time) we believed that this would prevent other teams from whitebox attacking our model. There is a way around this that I realized afterward, which we leverage in our attacks (refer to Breaking In Section in attacks for more details)
...
- To submit our defensive model we all pulled an all-nighter, which is the first time I pulled an all-nighter! I 100% would not recommend it! I would recommend getting blackout curtains though so you can sleep any time of the day!